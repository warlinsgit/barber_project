...
if(user_admin){
    const staffa = await pool.query('SELECT * FROM booking4 b LEFT JOIN staff_s st ON st.staff_id = b.staff_id order by id desc' );
...
//console.log('staffa=========', staffa);
  ...
  res.render('booking/book-confirmed',{staffa}); }
      ...
      const staffa = await pool.query('SELECT * FROM booking4 b LEFT JOIN staff_s st ON st.staff_id = b.staff_id WHERE b.client_id = ? order by id desc', [req.user.id]  );
    res.render('booking/book-confirmed',{staffa});
  }
});

// All booking service to be done - booking/service-booked
